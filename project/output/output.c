...
void print_product(FILE *p_out, Product *temp) {
  ...
  fprintf(p_out, "%s\n", temp->product_class);
  fprintf(p_out, "%s\n", temp->number);
  print_data(p_out, temp->install_data);
  if (temp->update_data != NULL)
    print_data(p_out, temp->update_data);
  fprintf(p_out, "\n");
}
...
void print_products(Product **one_class, size_t size_temp, FILE *pfile) {
  ...
  for (i = 0; i < size_temp; i++)
    print_product(pfile, one_class[i]);
}
...
void my_out(Product **all, size_t size_all, FILE *pfile) {
  ...
  for (size_t j = 0; j < size_all; j++) {
    ...
    if (flag_unique == false) {
      ...
      unique_clas = (char **)realloc(unique_clas,
                                     (size_unique_clas + 1) * sizeof(char *));
  ...
  for (size_t i = 0; i < size_unique_clas; i++) {
    ...
    for (size_t l = 0; l < size_all; l++) {
      ...
      if (strcmp(all[l]->product_class, unique_clas[i]) == 0 &&
          (all[l]->update_data == NULL ||
           (all[l]->update_data[0] == all[l]->install_data[0] &&
            all[l]->update_data[1] == all[l]->install_data[1] &&
            all[l]->update_data[2] == all[l]->install_data[2]))) {
