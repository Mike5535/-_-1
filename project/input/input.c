...
bool chek_string(char *temp, bool f) {
  ...
  bool flag = false;
  while (temp[i] != '\0') {
    if ((f || temp[i] < 'A' || temp[i] > 'Z') &&
        (f || temp[i] < 'a' || temp[i] > 'z') &&
        (temp[i] - '0' < 0 || temp[i] - '0' > 9))
      flag = true;
    i++;
  }
  return flag;
...
bool get_product(Product *temp, FILE *pfile) {
  ...
  if (chek_string(temp->name, false)) {
    ...
    temp->name = NULL;
    clean_input_product(
        pfile); // позволяет игнорировать оставшиеся данные неправильной записи
    return true;
...
Product *chek_null(Product *temp) {
  if (temp->name == NULL || temp->number == NULL ||
      temp->product_class == NULL || temp->install_data == NULL) {
    if (temp->name != NULL)
      free_product(temp);
    free(temp);
    temp = NULL;
  ...
  if (f1 == true) {
    do {
      ...
      all[i] = chek_null(all[i]);
             ...
             f2 == true); // две пустые строки конец записываемых данных
  }
  pi[0] = i;
  return all;
}